...
from trainer_reic import Trainer, TrainerCallback, LSTMSelector
...
def process_example(args, h, t, doc1, doc2):
    ...
    with open(f'{senemb_path}/train/codred-doc-senemb-doc1-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc1 = json.load(json_file)
    with open(f'{senemb_path}/train/codred-doc-senemb-doc2-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc2 = json.load(json_file)

    k1, intervals1, target_emb1, lst_emb1 = doc1['tokens'], doc1['intervals'], doc1['target_emb'], doc1['lst_emb']
    k2, intervals2, target_emb2, lst_emb2 = doc2['tokens'], doc2['intervals'], doc2['target_emb'], doc2['lst_emb']
    ...
    for i in range(len(lst_batch)):
        ...
        if batch[-1] == 'o':
            ...
            for doc1, doc2, l in dps:
                k1, intervals1, target_emb1, lst_emb1, k2, intervals2, target_emb2, lst_emb2 = process_example(args, h, t, doc1, doc2)
...
def process_example_infer(args, h, t, doc1, doc2):
    doc1_id = doc1
    doc2_id = doc2
    ...
    with open(f'{senemb_path}/dev/codred-doc-senemb-doc1-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc1 = json.load(json_file)
    with open(f'{senemb_path}/dev/codred-doc-senemb-doc2-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc2 = json.load(json_file)
...
def collate_fn_infer(batch, args, relation2id, tokenizer, redisd, sent_model):
    ...
    for doc1, doc2, l in dps:
        k1, intervals1, target_emb1, lst_emb1, k2, intervals2, target_emb2, lst_emb2 = process_example_infer(args, h, t, doc1, doc2)
        tokens_codre.append((k1, k2))
        intervals_codre.append((intervals1, intervals2))
        target_emb_codre.append((target_emb1, target_emb2))
        lst_emb_codre.append((lst_emb1, lst_emb2))
...
def process_example_test(args, h, t, doc1, doc2):
    doc1_id = doc1
    doc2_id = doc2
    ...
    with open(f'{senemb_path}/test/codred-doc-senemb-doc1-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc1 = json.load(json_file)
    with open(f'{senemb_path}/test/codred-doc-senemb-doc2-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc2 = json.load(json_file)
...
def collate_fn_test(batch, args, relation2id, tokenizer, redisd, sent_model):
    ...
    for doc1, doc2, l in dps:
        k1, intervals1, target_emb1, lst_emb1, k2, intervals2, target_emb2, lst_emb2 = process_example_test(args, h, t, doc1, doc2)
        tokens_codre.append((k1, k2))
        intervals_codre.append((intervals1, intervals2))
        target_emb_codre.append((target_emb1, target_emb2))
        lst_emb_codre.append((lst_emb1, lst_emb2))
...
class Codred(torch.nn.Module):
    def __init__(self, args, num_relations):
        ...
        if args.v2:
            ...
            self.v2 = False
            num_layer = 4
        self.matt = MatTransformer(h = 8 , d_model = self.d_model , hidden_size = 1024 , num_layers = num_layer , device = torch.device(0))
...
class CodredCallback(TrainerCallback):
    ...
    def collate_fn(self):
        return partial(collate_fn, args=self.args, relation2id=self.relation2id, tokenizer=self.tokenizer, redisd=self.redisd, sent_model=self.sent_model),\
               ...
               partial(collate_fn_test, args=self.args, relation2id=self.relation2id, tokenizer=self.tokenizer, redisd=self.redisd, sent_model=self.sent_model)
